...
for(const [type,checkpoint] of [['climb',2],['zipline',3],['bird',7],['leap',9]]){
...
h.checkpoint=5;c.rat.position.copy(h.actions.find(a=>a.type==='lizard').p);c.grab();c.keys.w=true;for(let i=0;i<605;i++){const ride=h.transit;if(ride){const target=ride.progress<.36?.65:ride.progress<.62?-.65:.65;const delta=target-(ride.offset||0);c.joy.x=Math.abs(delta)>.04?Math.sign(delta):0;}c.tickHeaven183(1/60);}c.joy.x=0;c.keys.w=false;assert(h.completed.lizard);assert.equal(h.checkpoint,6);
...
h.checkpoint=10;c.rat.position.set(6,.3,-70);c.grab();assert(c.home.stairwayPortrait183);assert(captures[0].includes('complete'));
