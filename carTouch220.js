...
(()=>{
 ...
 function attach(){const c=car77;if(owner===c)return;owner=c;targets=[];parts=[];selected=null;mirror=null;if(!c)return;vehicle219.attach(c);const a=c.cabin219;
  ...
  for(const [id,name,obj,upgrade]of [['radiator','Radiator',a.engine.getObjectByName('Radiator'),'cooling'],['filter','Air filter',filter,null],['plugs','Spark plugs',plugs,'engine'],['belt','Drive belt',belt,null],['brakes','Brake caliper',brake,'brakes'],...c.hubs.map((wheel,i)=>['wheel'+i,['Rear left','Front left','Rear right','Front right'][i]+' wheel',wheel,i===0?'tyres':null])]){
   ...
   const slot=new THREE.Mesh(obj.geometry||new THREE.BoxGeometry(.14,.49,.49),new THREE.MeshBasicMaterial({color:0xd8c48b,wireframe:true,transparent:true,opacity:.25,depthWrite:false}));slot.position.copy(p.position);slot.quaternion.copy(p.quaternion);slot.scale.copy(p.scale);p.parent.add(slot);p.slot=slot;slot.visible=false;
   const t=target('part-'+id,obj,()=> 'Hold '+name,()=>pickup(p));t.part=p;
   const st=target('slot-'+id,slot,()=> 'Refit '+name,()=>refit(p));st.part=p;
   if(!installed(p)){c.g.updateWorldMatrix(true,true);root.attach(obj);obj.position.fromArray(state()[id].position);slot.visible=true;}
   parts.push(p);
  }
 }
 ...
 function pickup(p){if(!active()||car77.riding||Math.abs(car77.speed)>.08||vehicle219.service)return false;if(held)return false;if(car77.ignition206)return sayToast('Switch the engine off first.');const t=targets.find(t=>t.id==='part-'+p.id);if(!valid(t))return false;
  root.updateWorldMatrix(true,true);root.attach(p.mesh);state()[p.id]={installed:false,position:p.mesh.position.toArray(),upgraded:!!state()[p.id]?.upgraded};p.slot.visible=true;held={part:p,owner:root,aim:null};selected=null;save();tutorial219.record('service');sayToast('Holding '+p.name+'. Move to carry it; tap its outlined slot to refit.');return true;
 }
 function refit(p){if(!held||held.part!==p||!valid(targets.find(t=>t.id==='slot-'+p.id))||car77.ignition206)return false;p.parent.add(p.mesh);p.mesh.position.copy(p.position);p.mesh.quaternion.copy(p.quaternion);p.mesh.scale.copy(p.scale);p.slot.visible=false;state()[p.id]={installed:true,upgraded:!!state()[p.id]?.upgraded};held=null;selected=null;save();tutorial219.record('service');sayToast(p.name+' refitted.');return true;}
 function drop(){if(!held)return;const p=held.part;if(held.owner===root){const forward=V(Math.sin(rat.rotation.y),0,Math.cos(rat.rotation.y));p.mesh.position.copy(rat.position).addScaledVector(forward,.35);p.mesh.position.y=rat.position.y+.08;p.mesh.quaternion.identity();persist(p);}held=null;selected=null;}
 ...
 window.carTouch220={attach,hit,current,pickup,refit,drop,upgrade,renderUI,get targets(){return targets;},get parts(){return parts;},get held(){return held;},get mirror(){return mirror;}};
})();
