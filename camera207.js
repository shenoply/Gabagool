...
(()=>{
 ...
 let activeCar=null,walking=null,lastOutside=false;
 ...
 const before=tickGameplayCamera;tickGameplayCamera=function(dt){
  ...
  const goal=target.clone().addScaledVector(best.direction,best.available);
  if(activeCar!==c||!lastOutside||!gameCam.ready||best.available<distance-.1)gameCam.pos.copy(goal);else gameCam.pos.lerp(goal,1-Math.exp(-dt*9));
