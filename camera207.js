...
(()=>{
 ...
 const before=tickGameplayCamera;tickGameplayCamera=function(dt){
  ...
  const presets=vi===5?{distance:4.8,pitch:.22,yaw:c.g.rotation.y+Math.PI}:vi===6?{distance:9.5,pitch:.34,yaw:c.g.rotation.y+Math.PI}:vi===7?{distance:15.5,pitch:.78,yaw:c.g.rotation.y+Math.PI}:{distance:Math.max(minimum,Number.isFinite(gameCam.distance)?gameCam.distance:minimum),pitch:THREE.MathUtils.clamp(gameCam.pitch,.30,1.2),yaw:gameCam.yaw};
  const distance=Math.max(minimum,presets.distance),pitch=presets.pitch,solids=wallSolids();
