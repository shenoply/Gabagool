(() => {
  ...
  function sourceCrawl108(actor, moving, dt) {
    ...
    constrainLimbs113(actor, moving);
    groundRenderedFeet112(actor);
    actor.g.updateMatrixWorld(true);
  }

  function constrainLimbs113(actor, moving) {
    const roots = ['ArmF.L_05','ArmF.R_010','Bone.019_024','Bone.023_029'];
    const mids = [
      'ArmF2.L_06','ArmF3.L_07','ArmF2.R_011','ArmF3.R_012',
      'Bone.020_025','Bone.021_026','Bone.024_030','Bone.025_031'
    ];
    const toes = ['ArmF4.L_08','ArmF4.R_013','Bone.022_027','Bone.026_032'];
    const settle = (names, rotationBlend) => {
      for (const name of names) {
        const bone = actor.bones?.[name];
        const restQ = actor.rest?.[name];
        const restP = actor.sourceRestPos108?.[name];
        if (bone && restQ) bone.quaternion.slerp(restQ, rotationBlend);
        if (bone && restP) bone.position.lerp(restP, .82);
      }
    };
    // Keep a restrained portion of the authored diagonal gait. The original
    // clip straightens the forearm so far that it reads as a second neck.
    settle(roots, moving ? .72 : .84);
    settle(mids, moving ? .78 : .88);
    settle(toes, moving ? .86 : .93);
  }

  function groundRenderedFeet112(actor) {
    const carrier = actor.carrier103;
    if (!carrier) return;
    if (!actor.skinned112) {
      actor.skinned112 = [];
      actor.model.traverse(o => { if (o.isSkinnedMesh) actor.skinned112.push(o); });
    }
    carrier.position.y = .02;
    actor.g.updateMatrixWorld(true);
    let lowest = Infinity;
    const v = new THREE.Vector3();
    for (const mesh of actor.skinned112) {
      const count = mesh.geometry?.attributes?.position?.count || 0;
      const step = Math.max(1, Math.ceil(count / 4200));
    ...
    if (Number.isFinite(lowest)) carrier.position.y += actor.g.position.y + .018 - lowest;
  }
