...
(()=>{
 ...
 function sourceHint257(id){
  return ({stick:'Yard scraps and fallen branches',pebble:'Yard paths and soil',string:'Salvage twine or craft from fibres',fiber:'Grass and plant scraps',foil:'Dumpster and roadside salvage',nail:'Straighten foil with the hammer',log230:'Chop a fallen branch with the axe',plank230:'Saw a seasoned branch at the home workbench',paint230:'Mix water, sap and leaves',water:'Home collector or outdoor water',sap:'Plants and damaged branches',leaf:'Garden and yard plants'})[id]||'Explore salvage, the yard and the dumpster.';
 }
 function needHtml(c){return Object.entries(c.needs||{}).sort(([ka,na],[kb,nb])=>(count(ka)>=na)-(count(kb)>=nb)).map(([k,n])=>{
  ...
  return '<button class="need251 '+(ok?'ok250':'miss250')+'" data-source257="'+k+'">'+ingredientIcon(k)+'<span><strong>'+esc(name)+'</strong><small>Have '+have+' · Need '+n+'</small></span></button>';
 }).join('');}
 ...
 function seedWorkshop251(){
  ...
  const rec=tableRecord255()||{x:-4.4,z:-1.9,rot:0};const legacy=hs?.placed?.find(e=>e.p?.id==='workbench');if(legacy?.m)legacy.m.visible=false;const g=new THREE.Group();g.name='Pip detailed crafting table';g.position.set(rec.x,0,rec.z);g.rotation.y=rec.rot||0;root.add(g);root.userData.workshop251=g;
  ...
  add(new THREE.BoxGeometry(1.55,.08,.28),woodM,-.25,1.92,.42,'Workshop wall shelf');
  for(let i=0;i<3;i++){add(new THREE.CylinderGeometry(.08,.09,.18,12),new THREE.MeshStandardMaterial({color:[0x8c9c78,0xb58b67,0x7f8f91][i],roughness:.55}),-.72+i*.28,2.05,.38,'Workshop jar');add(new THREE.CylinderGeometry(.085,.085,.025,12),metalM,-.72+i*.28,2.155,.38,'Jar lid');}
  const cloth=add(new THREE.BoxGeometry(.48,.015,.34),new THREE.MeshStandardMaterial({color:0x697d64,roughness:1}),.28,.905,-.28,'Workbench cloth');cloth.rotation.y=-.18;
  const fill=new THREE.PointLight(0xa7c7a4,.22,4);fill.position.set(.9,1.35,-.5);g.add(fill);
 ...
 function actions250(){
  ...
  const arr=[
   ...
   ['Tail whip',()=>{closeModal();whip67();}],
   ['Bite',()=>{closeModal();doBite();}],
   ['Roll',()=>{closeModal();doRoll();}],
   [rat?.userData?.crawl169?'Stand up':'Crawl',()=>{closeModal();toggleCrawl169();}],
 ...
 #workshopPanel251{position:fixed;z-index:90;inset:0;background:transparent;color:#f4ead6;font-family:system-ui;display:grid;grid-template-rows:auto minmax(130px,25vh) 1fr auto;overflow:hidden}
 ...
 #workshopScene253{position:relative;background:linear-gradient(90deg,rgba(24,33,28,.04),rgba(24,33,28,.18));overflow:hidden;pointer-events:none}
 ...
 #workshopBody251{overflow:auto;padding:12px 14px 18px;touch-action:pan-y;background:linear-gradient(#1c2a24f8,#17231efc);box-shadow:0 -10px 30px #0007}
