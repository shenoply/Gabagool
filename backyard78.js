...
function simplePool78(){
 const g=new THREE.Group();g.position.set(-15,0,33);g.name='Clean striped inflatable kids pool';
...
function recolor78(){root.traverse(m=>{if(!m.isMesh)return;const name=(m.userData.sourceMaterial78||'')+' '+m.name;if(/Peeling teal fence|Backyard perimeter fence/i.test(name)){m.material=new THREE.MeshStandardMaterial({color:0x617b79,roughness:.96});}});}
...
async function seed78(){
 ...
 const jobs=[
 ['picnic78.glb',a=>{const g=fitAsset78(cleanSupplied78(a.scene.clone(true),'picnic'),[14,0,26],{w:3.6,h:1.45,d:3.25});g.name='Backyard picnic table';solid78(g);y.table=g;addFind78('tableCrumbs','Picnic crumbs','crumbs',2,[14,1.53,26]);
  // A short rope gives a readable way onto the tabletop; benches remain jumpable.
  rope76(12.5,26,1.45,13,26);}],
 ...
 ['gnomes78.glb',a=>{for(const [x,z]of [[11.8,34.8],[27.4,22.8]]){const g=fitAsset78(cleanSupplied78(a.scene.clone(true),'gnome'),[x,0,z],{h:.82,uniform:1});g.name='Garden gnome landmark';solid78(g);}addFind78('gnomeButton','Gnome’s lost button','button',1,[27.6,.13,23.3]);}],
