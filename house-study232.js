...
const gardenView=new T.Mesh(new T.PlaneGeometry(3.4,3.4/1.5),gardenMaterial);gardenView.position.set(.165,1.98,-3.5);scene.add(gardenView);
// Smaller wreckage mixed with fallen timber rather than identical chunks.
for(let i=0;i<13;i++){const sliver=box(.06+(i%3)*.03,.04,.3+(i%4)*.23,5.5+(i%4)*.3,.06,-2.8+(i%5)*.37,dark,broken);sliver.rotation.y=i*1.1;}
for(let i=0;i<7;i++){const nail=mesh(new T.CylinderGeometry(.013,.013,.2,5),iron,broken);nail.position.set(6+(i%3)*.2,.04,-2+i*.13);nail.rotation.z=1.45;}
for(const x of [-7.45,7.45])box(.04,.3,10.2,x,.64,0,mat(0x696450));
