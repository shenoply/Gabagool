...
`;document.head.appendChild(style);
  ...
  function openYard201(selected=tracked||'alley'){
    ...
    modal('Pip’s Lane · Yard & city',`<div class="map201"><svg viewBox="0 0 100 100" preserveAspectRatio="none"><path d="M9 42H89V94H9Z" fill="#45623f" stroke="#a08959" stroke-width="1.5"/><rect x="17" y="11" width="68" height="25" fill="#767e75"/><path d="M48 44V34" stroke="#93988f" stroke-width="5"/><text x="51" y="8" text-anchor="middle" fill="#f5e5b9" font-size="3">ORIGINAL CITY</text></svg>${markers}<span class="player" style="left:${p.x}%;top:${p.y}%" title="Pip">●</span></div><p class="map201-note"><b>${d.name}</b><br>${d.detail}</p><div class="map201-list">${all.filter(d=>!d.unavailable).map(d=>`<button data-map201="${d.id}" class="${d.id===selected?'active':''}">${d.icon} · ${d.name}</button>`).join('')}</div>`,[['Track destination',()=>track(d.id),!!d.unavailable],['Objectives',openObjectives200],['Close',closeModal]]);
  ...
  openObjectives200=function(){const s=progress76();const items=[
    ...
    ['Explore the original city','Open · north gate','city','<p>Walk or drive through the streets of the uploaded city.</p>'],
