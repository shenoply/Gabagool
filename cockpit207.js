...
(()=>{
 ...
 #cockpit205 [data-icon207=camera]{font:600 10px/1.3 system-ui!important;width:66px!important;min-width:66px!important;flex:0 0 66px!important;border-radius:12px!important}#cockpit205 [data-icon207=camera]::after{display:none!important}
 ...
 #cockpit205{width:170px;bottom:max(18px,env(safe-area-inset-bottom))}#cockpit205 .cockpit205-quick{gap:6px;margin-bottom:10px}
 ...
 const before=tickWorld38;tickWorld38=function(dt){before(dt);const cameraButton=quick.querySelector('[data-icon207=camera]');if(cameraButton){const label=['Car view · Outside','Car view · Driver','Car view · Hands','Car view · Dash','Car view · Shoulder'][roadsterControls203.viewIndex];cameraButton.textContent=label;cameraButton.title=label+' — tap to change';cameraButton.setAttribute('aria-label',label+' — tap to change');}rail.classList.toggle('map-panel207',!cockpit205.drawer.hidden);const map=document.getElementById('miniMap171');if(map&&!map.querySelector('.mapTerrain207')){map.insertAdjacentHTML('afterbegin','<svg class="mapTerrain207" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true" style="position:absolute;inset:0;width:100%;height:100%;pointer-events:none;opacity:.55"><path d="M9 42H92V94H9Z" fill="#5e794b" stroke="#d0b784" stroke-width=".6"/><path d="M48 43V12" stroke="#b0aaa0" stroke-width="5"/><path d="m44 17 4-5 4 5" fill="none" stroke="#f0d896" stroke-width="1"/><text x="50" y="8" font-size="5" text-anchor="middle" fill="#fff1cc">ORIGINAL CITY ↑</text></svg>');}rail.dataset.inactive207=String(phase!=='scavenge'||photo.active||!gameplayActive());};
