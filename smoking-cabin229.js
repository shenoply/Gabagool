...
function createStudyCabin229({THREE,car,renderer,camera,getMode,onReach,onStatus,live=null}){
 ...
 let swingX=0,swingZ=0,velocityX=0,velocityZ=0,lastSpeed=0;
 function tick(dt,angle,moving,motion=null){t+=dt;
  const speed=motion?.speed??(live?live.speed:0),acceleration=motion?.acceleration??(speed-lastSpeed)/Math.max(dt,.001),lateral=motion?.lateral??speed*angle;lastSpeed=speed;
  // Two-axis damped pendulum. Vehicle acceleration drives it; turning the camera does not.
  let remaining=Math.min(dt,.1);while(remaining>0){const h=Math.min(remaining,1/120),ax=THREE.MathUtils.clamp(acceleration,-5,5),az=THREE.MathUtils.clamp(lateral,-5,5);velocityX+=(-28*Math.sin(swingX)-3.1*velocityX-ax*2)*h;velocityZ+=(-28*Math.sin(swingZ)-3.1*velocityZ+az*2)*h;swingX+=velocityX*h;swingZ+=velocityZ*h;remaining-=h;}
  swingX=THREE.MathUtils.clamp(swingX,-.55,.55);swingZ=THREE.MathUtils.clamp(swingZ,-.55,.55);charm.rotation.set(swingX,0,swingZ);

  const kph=motion?Math.abs(motion.speed)*3.6:live?Math.abs(live.speed)*3.6:(moving?18+Math.sin(t*.8)*3:0),gear=live?(live.speed<-.08?'R':kph<.3?'N':kph<10?'1':kph<21?'2':'3'):(moving?'2':'N'),engine=live?live.ignition206:moving,service=live?vehicle219.data():{temperature:engine?78:22,oil:100,coolant:100};
 ...
 return {root,charm,get swing(){return {x:swingX,z:swingZ,vx:velocityX,vz:velocityZ};},door,handle,windowSwitch,knobs,activate,tick,needles,get state(){return {station,radioOn,doorOpen,windowOpen,busy:!!action};}};
}
