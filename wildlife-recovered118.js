(() => {
  ...
  function addMap171() {
    ...
    map.innerHTML = '<b>Yard map</b><i data-id="home" title="Pip’s home">⌂</i><i data-id="den" aria-label="Lizard den" title="Lizard den">🦎</i><i data-id="bird" aria-label="Osprey transport" title="Osprey transport">🦅</i><i data-id="pool" title="Old pool">≈</i><i data-id="dumpster" title="Dumpster">▣</i><i data-id="car" title="Convertible">C</i><i data-id="alley" title="Unlocked alley route">A</i><i data-id="mission-vinyl" aria-label="Lost vinyl" title="Lost vinyl">♪</i><i data-id="mission-pool" aria-label="Pool key" title="Pool key">🔑</i><i data-id="pip">●</i><small class="mapLegend200">A alley · C car</small>';
    map.style.cssText = 'display:none;position:fixed;right:12px;top:104px;width:138px;height:138px;border:2px solid #e7dbc0;border-radius:18px;background:linear-gradient(90deg,#ffffff0d 1px,transparent 1px),linear-gradient(#ffffff0d 1px,transparent 1px),#29463dee;background-size:23px 23px;color:#f7eedc;z-index:24;pointer-events:none;overflow:hidden;font:11px system-ui';
    ...
    map.querySelector('[data-id="home"]').style.color = '#f4d987'; map.querySelector('[data-id="den"]').style.color = '#a8df9f'; map.querySelector('[data-id="bird"]').style.color = '#b9def5'; map.querySelector('[data-id="pool"]').style.color = '#8bd4dd'; map.querySelector('[data-id="dumpster"]').style.color = '#b8aaa0'; map.querySelector('[data-id="car"]').style.cssText += ';color:#2b3f38;background:#f0c75e;border-radius:50%;width:17px;height:17px;line-height:17px;text-align:center;font-weight:900'; map.querySelector('[data-id="alley"]').style.cssText += ';color:#213a2e;background:#b8f0a3;border-radius:50%;width:17px;height:17px;line-height:17px;text-align:center;font-weight:900'; map.querySelector('[data-id="pip"]').style.color = '#ffead0';
    ...
    if(!$('objectives200')){const objectives=document.createElement('button');objectives.id='objectives200';objectives.textContent='☰ Objectives';objectives.onclick=()=>openObjectives200();document.body.appendChild(objectives);}
    const style = document.createElement('style'); style.textContent = '#mute{top:224px!important;z-index:28!important}#miniMap171 .mapLegend200{position:absolute;left:7px;bottom:3px;padding:1px 4px;border-radius:5px;background:#1e352fd9;color:#f5e7c7;font:700 8px system-ui}#objectives200{display:none;position:fixed;z-index:28;padding:8px 11px;border:1px solid #ead9ae;border-radius:10px;background:#29463eee;color:#fff0d2;font:700 12px system-ui;pointer-events:auto}.menu-open65 #objectives200,.menu-open65 #geckoWhistle193,.menu-open65 #heavenLaunch186{display:none!important}.objectives200{display:grid;gap:10px}.objectives200 section{padding:13px;border-radius:12px;background:#e4e8d8}.objectives200 section b{font-size:18px}.objectives200 section p{margin:7px 0!important}.objectives200 small{color:#637263}.photo #hud,.photo #settings,.photo #survival,.photo #weatherBadge,.photo #mute,.photo #pad,.photo #cameraTools,.photo #miniMap171,.photo #objectives200,.photo #prompt{display:none!important}'; document.head.appendChild(style);
  }
  ...
  tickWorld38 = function tickPolish171(dt) {
    ...
    const map = $('miniMap171'); if (map) { const mapVisible=phase === 'scavenge' && !photo.active;map.style.display=mapVisible?'block':'none';const objectives=$('objectives200');if(objectives){objectives.style.display=mapVisible&&$('modal').style.display!=='flex'?'block':'none';if(mapVisible){const rect=map.getBoundingClientRect();objectives.style.top=Math.round(rect.bottom+7)+'px';objectives.style.right=Math.max(10,Math.round(innerWidth-rect.right))+'px';}}if(rat) mapPoint171(map.querySelector('[data-id="pip"]'), rat.position.x, rat.position.z); mapPoint171(map.querySelector('[data-id="home"]'), 0, -2);mapPoint171(map.querySelector('[data-id="pool"]'),-12.5,30.7);mapPoint171(map.querySelector('[data-id="dumpster"]'),8.7,-10.8); const den = wildlife88?.den153 ? DEN_POS_155 : new THREE.Vector3(-18.75, 0, 24.75); mapPoint171(map.querySelector('[data-id="den"]'), den.x, den.z); const bird=typeof rideActor71==='function'?rideActor71():null,birdMark=map.querySelector('[data-id="bird"]');if(bird&&birdMark){birdMark.style.display='block';mapPoint171(birdMark,bird.g.position.x,bird.g.position.z);}else if(birdMark)birdMark.style.display='none';const carMark=map.querySelector('[data-id="car"]');if(car77?.owner===root&&carMark){carMark.style.display='block';mapPoint171(carMark,car77.g.position.x,car77.g.position.z);}else if(carMark)carMark.style.display='none'; const mission=home?.yard76,active=!!mission?.missionsActive&&!mission.jobs,alley=map.querySelector('[data-id="alley"]'),alleyOpen=!!mission?.jobs||!!home?.routes?.courtyard;if(alley){alley.style.display='block';alley.title='Alley route / rooftop gate';mapPoint171(alley,30.6,16.2);}const targets=[['mission-vinyl',yard76?.vinyl?.m?.position,!mission?.vinyl],['mission-pool',yard76?.poolFind?.m?.position,!mission?.pool]];for(const [id,pos,incomplete] of targets){const mark=map.querySelector(`[data-id="${id}"]`),show=active&&incomplete&&!!pos;if(mark)mark.style.display=show?'block':'none';if(show)mapPoint171(mark,pos.x,pos.z);} }
  ...
  control = function controlPlayyard177(dt, options) {
    const next = phase === 'scavenge' ? { ...options, bounds: [-86, 94, -25, 145] } : options;
  ...
  mapPoint171=function(el,x,z){if(!el)return;const wide=window.mapWide209?.();el.style.left=Math.max(6,Math.min(94,wide?6+(x+86)/180*88:6+(x+22)/54*88))+'%';el.style.top=Math.max(6,Math.min(94,wide?94-(z+25)/170*88:94-(z+14)/84*88))+'%';};
