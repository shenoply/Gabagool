...
function makeCanoe212(){
 ...
 // Close the hull beneath the deck and across both narrow ends.
 function hullPanel(points,name){const geo=new THREE.BufferGeometry();geo.setAttribute('position',new THREE.Float32BufferAttribute(points.flat(),3));geo.setIndex([0,1,2,0,2,3]);geo.computeVertexNormals();const mat=wood[0].clone();mat.side=THREE.DoubleSide;const mesh=new THREE.Mesh(geo,mat);mesh.name=name;g.add(mesh);}
 for(let i=0;i<stations.length-1;i++){const a=stations[i],b=stations[i+1];hullPanel([[-a[1]*.67,a[2],a[0]],[a[1]*.67,a[2],a[0]],[b[1]*.67,b[2],b[0]],[-b[1]*.67,b[2],b[0]]],'Sealed hull bottom');}
 for(const s of [stations[0],stations[stations.length-1]]){hullPanel([[-s[1]*.67,s[2],s[0]],[s[1]*.67,s[2],s[0]],[s[1],s[2]+.28,s[0]],[-s[1],s[2]+.28,s[0]]],'Closed hull end');beam([-s[1],s[2]+.28,s[0]],[s[1],s[2]+.28,s[0]],.026,wood[1]);}
