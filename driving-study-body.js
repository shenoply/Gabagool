...
function createStudyInsideBody(THREE,mesh){
 ...
 const arrays=Object.fromEntries(keys.map(k=>[k,Array.from(attrs[k].array)])),patches=[];
 while(next.size){const start=next.keys().next().value,loop=[start];let at=start;for(let n=0;n<10000;n++){const list=next.get(at);if(!list?.length)break;const to=list.pop();if(!list.length)next.delete(at);at=to;if(at===start)break;loop.push(at);}if(at!==start||loop.length<3)continue;
  ...
  for(let k=0;k<vertices.length;k++)keep.push(vertices[(k+1)%vertices.length],vertices[k],center);patches.push({center,vertices});
 }
 ...
 function sync(){const p=inside.attributes.position;p.array.set(full.attributes.position.array);for(const {center,vertices}of patches)for(let k=0;k<3;k++)p.array[center*3+k]=vertices.reduce((n,i)=>n+p.array[i*3+k],0)/vertices.length;p.needsUpdate=true;smoothStudyNormals(THREE,inside);}
