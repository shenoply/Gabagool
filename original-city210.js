...
(function(scope){
 function buildCityCollision210(THREE,group){
  ...
  group.traverse(mesh=>{if(!mesh.isMesh||!mesh.geometry||mesh.material?.name==='RoadsGround')return;
   ...
   for(let i=0;i<(idx?idx.count:p.count);i+=3){for(let k=0;k<3;k++)points[k].fromBufferAttribute(p,idx?idx.getX(i+k):i+k).applyMatrix4(mesh.matrixWorld);
    ...
    for(const y of [.22,.48,.75]){const hits=[];for(let k=0;k<3;k++){const a=points[k],b=points[(k+1)%3];if((a.y<=y&&b.y>y)||(b.y<=y&&a.y>y)){const t=(y-a.y)/(b.y-a.y);hits.push({x:a.x+(b.x-a.x)*t,z:a.z+(b.z-a.z)*t});}}
