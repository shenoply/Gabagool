...
(function(scope){
 function buildCityCollision210(THREE,group){
  group.updateWorldMatrix(true,true);const bounds=new THREE.Box3().setFromObject(group),segments=[],cells=new Map(),seen=new Set(),cellSize=10;
  ...
  group.traverse(mesh=>{if(!mesh.isMesh||!mesh.geometry)return;
   ...
   for(let i=0;i<(idx?idx.count:p.count);i+=3){for(let k=0;k<3;k++)points[k].fromBufferAttribute(p,idx?idx.getX(i+k):i+k).applyMatrix4(mesh.matrixWorld);
    ...
    for(const y of [.08,.24,.52,.90]){const hits=[];for(let k=0;k<3;k++){const a=points[k],b=points[(k+1)%3];if((a.y<=y&&b.y>y)||(b.y<=y&&a.y>y)){const t=(y-a.y)/(b.y-a.y);hits.push({x:a.x+(b.x-a.x)*t,z:a.z+(b.z-a.z)*t});}}
