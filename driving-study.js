(()=>{
 ...
 new THREE.GLTFLoader().load('rat-animation-pack67.glb',asset=>{
  ...
  study.model.traverse(mesh=>{if(mesh.isSkinnedMesh)firstPersonMeshes.push(createStudyInsideBody(THREE,mesh));});window.drivingStudy={scene,camera,car,study,renderer,choose,cabin};choose(mode);
 ...
 canvas.onpointermove=e=>{const old=pointers.get(e.pointerId);if(!old)return;const dx=e.clientX-old.x,dy=e.clientY-old.y;pointers.set(e.pointerId,{x:e.clientX,y:e.clientY});if(pointers.size===1){yaw-=dx*.006;if(mode==='cockpit')yaw=THREE.MathUtils.clamp(yaw,-1.5,1.5);pitch=THREE.MathUtils.clamp(pitch+dy*.004,mode==='cockpit'?-.45:-.1,1.35);}else{const [a,b]=[...pointers.values()],d=Math.hypot(a.x-b.x,a.y-b.y);if(pinch){if(mode==='cockpit')firstPersonFov=THREE.MathUtils.clamp(firstPersonFov*pinch/d,55,100);else distance=THREE.MathUtils.clamp(distance*pinch/d,.25,6);}pinch=d;}};
 ...
 const mirrorStudy=createStudyMirrors({THREE,scene,car,renderer,camera,onSelect:pivot=>{if(!study)return;choose('cockpit');study.model.updateMatrixWorld(true);const eye=study.bones.Head.getWorldPosition(V()).add(V(0,.022,.025)),delta=pivot.getWorldPosition(V()).sub(eye);yaw=Math.atan2(delta.x,delta.z);pitch=-Math.atan2(delta.y,Math.hypot(delta.x,delta.z));},onStatus:text=>$('status').textContent=text});
