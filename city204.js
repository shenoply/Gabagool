...
(()=>{
 ...
 const inCity=p=>p.z>=40&&p.z<=66&&p.x>=-15&&p.x<=27;
 ...
 const blocked=blockedCar77;blockedCar77=function(p,r=.39){if(phase==='scavenge'&&p.z>36.3)return !inExtension(p)||citySolid(p,r);return blocked(p,r);};
 const resolve=resolveGeometry62;resolveGeometry62=function(p,before){if(phase==='scavenge'&&p.z>36.3&&inExtension(p)){if(citySolid(p,.2))p.copy(before);return;}resolve(p,before);};
 ...
 const controlBefore=control;control=function(dt,options){if(phase==='scavenge'&&city?.owner===root&&options?.bounds){options={...options,bounds:[-21,31,-14,66],ground:options.ground};const ground=arguments[1].ground;options.ground=(x,z)=>z>36.3?0:ground?ground(x,z):0;const before=rat.position.clone(),result=controlBefore(dt,options);if(rat.position.z>36.3&&!inExtension(rat.position))rat.position.copy(before);return result;}return controlBefore(dt,options);};
 ...
 window.city204={seed,route,inExtension,citySolid,get state(){return city;},release,tune,signal};
})();
