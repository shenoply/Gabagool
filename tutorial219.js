...
(()=>{
 ...
 function draw(){if(!chapter)return;const step=chapters[chapter][index];card.hidden=false;card.innerHTML='<strong>'+chapter[0].toUpperCase()+chapter.slice(1)+' · '+(index+1)+' / '+chapters[chapter].length+' — '+step[1]+'</strong><p>'+step[2]+'</p><p id=lessonHint221>'+readiness()+'</p><button id="lessonNext219">'+(passed?'Done ✓ · Next':'Try it first')+'</button><button id="lessonPause219">Pause</button>';$('lessonNext219').disabled=!passed;$('lessonNext219').onclick=next;$('lessonPause219').onclick=pause;clearHighlight();card.style.top='max(10px,env(safe-area-inset-top))';document.querySelectorAll('button').forEach(b=>{if(eventName(b)===step[0]&&!b.hidden&&b.offsetParent!==null)b.classList.add('tutorial-controls219');});}
